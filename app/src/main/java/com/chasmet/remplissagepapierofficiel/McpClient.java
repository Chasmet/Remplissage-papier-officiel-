...
public final class McpClient {
    ...
    public static void uploadPreviewImageBlocking(String endpoint, String token,
        ...
        if (jpegBytes == null || jpegBytes.length < 100 || jpegBytes.length > 1_500_000) {
            throw new IllegalArgumentException("Prévisualisation JPEG invalide ou trop volumineuse");
        }
        ...
        try {
            ...
            String body = readBody(stream);
            if (code < 200 || code >= 300) {
                throw new IllegalStateException("Prévisualisation refusée (HTTP " + code + ") : "
                        + abbreviate(body, 800));
            }
            ...
            if (!root.optBoolean("ok", false)) {
                throw new IllegalStateException(
                        root.optString("error", "Prévisualisation refusée par le serveur"));
            }
        } finally {
            if (connection != null) connection.disconnect();
        }
